...
/**
 * Enables interoperability between OpenCV and Thrust.
 * Source:
 *    https://github.com/opencv/opencv/tree/master/samples/cpp/tutorial_code/gpu/gpu-thrust-interop
 */
...
/*
    @Brief step_functor is an object to correctly step a thrust iterator according to the stride of
   a matrix
*/
//! [step_functor]
template <typename T>
...
//! [begin_itr]
/*
    @Brief GpuMatBeginItr returns a thrust compatible iterator to the beginning of a GPU mat's
   memory.
    @Param mat is the input matrix
    @Param channel is the channel of the matrix that the iterator is accessing.  If set to -1, the
   iterator will access every element in sequential order
*/
template <typename T>
